...
if __name__ == '__main__':
    ...
    with h5py.File('opensignals_98D391FD575B_2021-06-21_09-51-53', 'r') as f:
        # get the minimum value
        print(f.keys())
        key = list(f.keys())[0]
        analogData = list(f[key])

    t = numpy.linspace(0, len(analogData) / 1000, len(analogData))
    plt.figure(figsize=(50, 4))
    plt.plot(t, analogData)
    plt.show()
    ...
    t = numpy.linspace(0, len(new_data) / 1000, len(new_data))
    plt.figure(figsize=(50, 4))
    plt.plot(t, new_data)
    plt.show()
